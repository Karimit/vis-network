...
class RepulsionSolver {
  ...
  solve() {
    ...
    const b = 4 / 3;
    const repulsionFactor = this.options.repulsionFactor ?? 1
    ...
    for (let i = 0; i < nodeIndices.length - 1; i++) {
      ...
      for (let j = i + 1; j < nodeIndices.length; j++) {
        ...
        const nodeDistance = Math.max(node1.distanceToBorder(), node2.distanceToBorder())
        const a = -2 / 3 / nodeDistance;
        ...
        if (distance < 2 * nodeDistance) {
          ...
          forces[node1.id].x -= fx * repulsionFactor;
          forces[node1.id].y -= fy * repulsionFactor;
          forces[node2.id].x += fx * repulsionFactor;
          forces[node2.id].y += fy * repulsionFactor;
        }
      }
    }
  }
