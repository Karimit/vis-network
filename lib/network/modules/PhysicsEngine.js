import RepulsionSolver from "./components/physics/RepulsionSolver";
import DragSolver from "./components/physics/DragSolver"
import BoundarySolver from "./components/physics/BoundarySolver";
...
class PhysicsEngine {
  ...
  setOptions(options) {
    if (options !== undefined) {
      if (options === false) {
        ...
        // set the timestep
        this.timestep = this._determineTimestep()
      }
    }
    this.init();
  }
   ...
   */
  init() {
    const options = this.options.repulsion;
    this.dragSolver = new DragSolver(this.body, this.physicsBody, this.options.drag)
    this.boundarySolver = new BoundarySolver(this.body, this.physicsBody, this.canvas, options)
  ...
  initPhysics() {
    if (this.physicsEnabled === true && this.options.enabled === true) {
      if (this.options.stabilization.enabled === true) {
        ...
        // Don't fit on the initial render
        // this.body.emitter.emit("fit", {}, this.layoutFailed); // if the layout failed, we use the approximation for the zoom
        this.body.emitter.emit("spreadNodes")
        this.startSimulation();
      }
    } else {
      this.ready = true;
  ...
  physicsStep() {
    ...
    this.dragSolver.solve()
    this.boundarySolver.solve()
    this.moveNodes();
  }
