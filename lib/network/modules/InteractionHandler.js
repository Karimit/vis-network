...
class InteractionHandler {
  ...
  onDragStart(event) {
    ...
    this.body.emitter.emit('dragStarted', pointer)
  ...
  onDrag(event) {
    ...
    this.body.emitter.emit('drag', pointer)
    ...
    for (const nodeId in this.body.nodes) {
      if (Object.prototype.hasOwnProperty.call(this.body.nodes, nodeId)) {
        selections.push(this.body.nodes[nodeId])
      }
    }

    if (selections && selections.length /*&& this.options.dragNodes === true*/) {
  ...
  onDragEnd(event) {
    ...
    const pointer = this.getPointer(event.center);
    this.body.emitter.emit("startSimulation");
    this.body.emitter.emit('dragEnded', pointer)
