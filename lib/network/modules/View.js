...
class View {
  ...
  spreadNodes() {
    ...
    const numnodes = Math.floor(canvasHeight / 70)
    const space = canvasHeight % 70
    const spacePerNode = Math.floor(space / numnodes)
    ...
    for (const nodeId in this.body.nodes) {
      ...
      if (Object.prototype.hasOwnProperty.call(this.body.nodes, nodeId)) {
        ...
        node.y = (canvasHeight / -2) + randomIntFromInterval(0, spacePerNode) + (70 * yIndex)

        if (nodeIndex % 2) {
          // go right
          node.x = currentX + node.options.widthConstraint.maximum
        } else {
            // go left
            node.x = -node.options.widthConstraint.maximum - currentX
        }
