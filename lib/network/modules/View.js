...
class View {
  ...
  constructor(body, canvas) {
    ...
    this.body.emitter.on("spreadNodes", this.spreadNodes.bind(this));
  ...
  /**
   * Sets nodes initial positions
   */
  spreadNodes() {
    function randYInsideContainer(canvasHeight) {
      function randomIntFromInterval(min, max) { // min and max included 
        return Math.floor(Math.random() * (max - min + 1) + min);
      }

      const nodeRadius = 80
      const halfHeight = ((canvasHeight - nodeRadius) / 2)
      return randomIntFromInterval(-halfHeight, halfHeight)
    }
    ...
    for (const nodeId in this.body.nodes) {
      if (Object.prototype.hasOwnProperty.call(this.body.nodes, nodeId)) {
        ...
        node.y = randYInsideContainer(canvasHeight)
        console.log(node.y)
        if (nodeIndex % 2) {
          // go right
          node.x = (canvasWidth / 2) + node.options.widthConstraint.maximum
          // node.y = ys[nodeIndex]
          xAdjustFactor = 1
        } else {
            // go left
            node.x = -node.options.widthConstraint.maximum - (canvasWidth / 2)
            // node.y = -randYInsideContainer(canvasHeight)
            // node.y = ys[nodeIndex]
            xAdjustFactor = -1
        }
         
        while (processedNodes.some(n => n.intersectsWith(node, 80))) {
          node.y = randYInsideContainer(canvasHeight)
          node.x += xAdjustFactor * 80
          console.log(node.id, 'adjusting', node.y, node.x)
        }
    ...
    console.log('nodes', processedNodes)
